...
Procedure VisibleManagementAtServer()
	...
	If Object.RestrictionByProductsAndServicesVariant = RestrictionByProductsAndServicesVariant Then
		Items.ProductsAndServicesGroupsPriceGroupsClarificationValue.Title = NStr("en='Products and services'; ru = 'Номенклатура'");
		...
		Items.DecorationHelpClarification.Title = NStr("en='Fill in the refinements if it is required to apply discount amount to certain products or product groups which is different from the main amount. If the list is not completed, basic discount will be used for all products and services.';
			|ru = 'Заполните уточнения, если требуется, чтобы у определённых товаров или групп товаров значение скидки отличалось от основного значения. Если список не заполнен, то для всех позиций номенклатуры используется основное значение скидки.'");
	...
	ElsIf Object.RestrictionByProductsAndServicesVariant = RestrictionVariantByProductsAndServicesGroups Then
		...
		Items.ProductsAndServicesGroupsPriceGroupsClarificationValue.Title = NStr("en='Products and services category'; ru = 'Категория номенклатуры'");
		
		Items.DecorationHelpClarification.Title = NStr("en='Fill in the refinements if it is required to apply discount amount to certain products and services groups which is different from the main amount. If the list is not completed, basic discount will be used for all products and services groups.';
			|ru = 'Заполните уточнения, если требуется, чтобы у товаров определённых категорий значение скидки отличалось от основного значения. Если список не заполнен, то для всех категорий используется основное значение скидки.'");
	...
	ElsIf Object.RestrictionByProductsAndServicesVariant = VariantRestrictionByPriceGroups Then
		Items.ProductsAndServicesGroupsPriceGroupsClarificationValue.Title = NStr("en='Price group'; ru = 'Ценовая группа'");
		Items.ProductsAndServicesGroupsPriceGroupsClarificationValue.TypeRestriction = New TypeDescription("CatalogRef.PriceGroups");
		
		Items.DecorationHelpClarification.Title = NStr("en='Fill in the refinement if it is required to assign discount amount to the products of certain price groups which is different from the main amount. If the list is not completed, then basic discount will be applied for all price groups.';
			|ru = 'Заполните уточнения, если требуется, чтобы у товаров определённых ценовых групп значение скидки отличалось от основного значения. Если список не заполнен, то для всех ценовых групп используется основное значение скидки.'");
...
Procedure RestrictionByProductsAndServicesVariantOnChange(Item)
	...
	If Object.ProductsAndServicesGroupsPriceGroups.Count() > 0 Then
		...
		ShowQueryBox(Description, NStr("en='Table of refinements will be cleared. Continue?'; ru = 'Таблица уточнений будет очищена. Продолжить?'"), QuestionDialogMode.YesNo,,DialogReturnCode.No,"Change refining option");
	Else
		RestrictionByProductsAndServicesVariantBeforeChange = Object.RestrictionByProductsAndServicesVariant;
		VisibleManagementAtServer();
	EndIf;
...
Procedure RecipientsVisibleSetupAtServer()
	...
	If Object.Purpose = PurposeRetail Then
		...
		Items.DecorationHelpRecipients.Title = NStr("en = 'Complete the list of warehouses if discount (extra charge) will be provided only in certain warehouses.
			...
			|ru = 'Заполните список складов, если скидка (наценка) будет предоставляться только на определённых складах.
	...
	ElsIf Object.Purpose = PurposeWholesale Then
		...
		Items.DecorationHelpRecipients.Title = NStr("en = 'Complete the list of counterparties if the discount (extra charge) will be provided only to a certain list of counterparties.
			...
			|ru = 'Заполните список контрагентов, если скидка (наценка) будет предоставляться только определённому списку контрагентов.
	...
	Else
		...
		Items.DecorationHelpRecipients.Title = NStr("en = 'Complete the list of counterparties or warehouses if the discount (extra charge) will be provided only to a specific list of counterparties (wholesale)
			...
			|ru = 'Заполните список контрагентов или складов, если скидка (наценка) будет предоставляться только определённому списку контрагентов (опт)
