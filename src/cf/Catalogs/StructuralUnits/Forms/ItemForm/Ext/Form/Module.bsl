...
Procedure RetailPriceKindOnChange(Item)
	...
	If Not DataStructure.PriceCurrency = DataStructure.NationalCurrency Then
		...
		MessageText = NStr("en='Specify national currency (%NatCurrency%) for the ""%PriceKind%"" price kind for retail structural unit.';
							|ru='У вида цен ""%PriceKind%"", для розничной структурной единицы, должна быть задана национальная валюта (%NatCurrency%).'");
