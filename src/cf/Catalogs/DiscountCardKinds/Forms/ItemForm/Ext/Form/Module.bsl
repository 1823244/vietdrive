...
Function GetIntervalView(TabularSection, CurrentRow)
	...
	If NextString = Undefined Then
		IntervalView = StringFunctionsClientServer.SubstituteParametersInString(NStr("en = 'From %1 %2'; ru = 'От %1 %2'"),
			TrimAll(CurrentRow.LowerBound), AccountingCurrency);
	ElsIf CurrentRow.LowerBound = NextString.LowerBound Then
		IntervalView = NStr("en = 'ERROR: such lower limit already exists!'; ru = 'ОШИБКА: такая нижняя граница уже есть!'");
	Else
		IntervalView = StringFunctionsClientServer.SubstituteParametersInString(NStr("en = 'From %1 to %2 %3'; ru = 'От %1 до %2 %3'"),
			TrimAll(CurrentRow.LowerBound), TrimAll(NextString.LowerBound), AccountingCurrency);
	EndIf;
