...
Procedure SetItemsVisible()
	...
	If PriceCalculationMethod = "CalculatedDynamically" Then
		...
		Items.PriceCalculationMethod.ToolTip = NStr("en = 'Prices are not stored.
			...
			|ru = 'Цены не хранятся. Цены в документах пересчитываются автоматически относительно базовой цены.'");
	...
	ElsIf PriceCalculationMethod = "Calculated" Then
		...
		Items.PriceCalculationMethod.ToolTip = NStr("en = 'Prices are assigned and stored for each products and services item.
			...
			|ru = 'Цены применяются и хранятся для каждой позиции товаров и услуг.
			|Цены могут быть рассчитаны относительно базовой цены с помощью механизма формирования цен.'");
	...
	Else
		...
		Items.PriceCalculationMethod.ToolTip =  NStr("en = 'Prices are assigned and stored for each products and services item. 
			...
			|ru = 'Цены применяются и хранятся для каждой позиции товаров и услуг.
			|На основе этого вида цен могут быть рассчитаны другие виды цен.'");
