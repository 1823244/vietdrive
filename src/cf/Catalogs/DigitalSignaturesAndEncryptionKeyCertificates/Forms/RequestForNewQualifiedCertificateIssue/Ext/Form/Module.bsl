...
Procedure InstructionNavigationRefProcessing(Item, URL, StandardProcessing)
	...
	If ThisIsIndividualEntrepreneur Then
		...
		|3. Copy of the identification document of certificate owner.';ru='Комплект
		...
		|3. Копия документа, удостоверяющего личность владельца сертификата.'");
	Else
		WarningText =
			NStr("en=""Set
		|of documents: 1. Application for certificate issue (prepared in the previous step).
		|2. Copy* of the Tax Authority Registration Certificate (TIN).
		|3. Copy* of the identification document of company representative indicated for certificate issue.
		|4. Copy* of the document confirming the authority of
		...
		|документов: 1. Заявление на выпуск сертификата (подготовленное на предыдущем шаге).
		|2. Копия* свидетельства о постановке на учет в налоговом органе (ИНН).
		|3. Копия* документа, удостоверяющего личность представителя организации, указанного для выпуска сертификата.
		|4. Копия* документа, подтверждающего полномочия
...
Procedure OnChangingDocumentTypeOnServer()
	...
	If DocumentKind = "" Then
		DocumentKind = "21";
	EndIf;
	
	If DocumentKind = "21" Then
...
Function PreparedDocument()
	...
	DocumentViewPresentation = Items.DocumentKind1.ChoiceList.FindByValue(DocumentKind).Presentation;
	DocumentViewPresentation = Lower(Left(DocumentViewPresentation, 1)) + Mid(DocumentViewPresentation, 2);
	
	DocumentNumberPresentation =
		?(DocumentKind = "21", AttributePresentationRFPassportNumber(DocumentNumber), DocumentNumber);
...
Function XMLStatement(Ticket)
	...
	XMLWriter.WriteAttribute("VersProg", "1C");
		...
		XMLWriter.WriteStartElement("EDSOwner");
			...
			XMLWriter.WriteAttribute("CodeDocKind", DocumentKind);
...
Procedure FillAttributesStatements(All = False)
	...
	String.Value = Items.DocumentKind1.ChoiceList.FindByValue(DocumentKind).Presentation;
...
Procedure FillApplicationsList()
	...
	For Each Settings IN Settings Do
		If Settings.ID = "MicrosoftEnhanced" Then
