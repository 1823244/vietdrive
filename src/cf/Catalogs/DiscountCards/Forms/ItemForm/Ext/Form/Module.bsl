...
Procedure FillInInformationOnDiscountCard()
	...
	If ValueIsFilled(AdditionalParameters.PeriodPresentation) Then
		Items.SalesAmountOnDiscountCard.Title = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Sales amount (%1)' ru = 'Сумма продаж (%1)'"), AdditionalParameters.PeriodPresentation);
	Else
		Items.SalesAmountOnDiscountCard.Title = NStr("en = 'Sales amount'; ru = 'Сумма продаж'");
	EndIf;
