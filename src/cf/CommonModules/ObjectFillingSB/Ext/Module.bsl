...
Procedure SupplementWorkOrderSettings(ValuesFromSettings, DocumentObject, FillingData)
	...
	WorkOrderAttributes = New Map;
	WorkOrderAttributes["WorkKindPosition"]		= Enums.AttributePositionOnForm.InHeader;
	WorkOrderAttributes["UseProducts"]			= True;
	WorkOrderAttributes["UseConsumerMaterials"]	= False;
	WorkOrderAttributes["UseMaterials"]			= False;
	WorkOrderAttributes["UsePerformerSalaries"]	= False;
	
	For Each KeyAndValue In WorkOrderAttributes Do
...
Procedure SupplementShipmentDatePositionInCustomerOrder(ValuesFromSettings, DocumentObject)
	...
	If DocumentObject.OperationKind = Enums.OperationKindsCustomerOrder.WorkOrder Then
		Return;
	EndIf;
