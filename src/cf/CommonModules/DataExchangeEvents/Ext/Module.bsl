...
Procedure AddRecipients(Object, Nodes) Export
	...
	For Each Item IN Nodes Do
		...
		Except
			...
			MessageString = NStr("en='Registration of object [DescriptionFull] is not specified for exchange plan content [ExchangePlanName]';
								 |ru='Для состава плана обмена [ExchangePlanName] не указана регистрация объекта [DescriptionFull]'");
...
//                  database such as catalog document, catalog item,
...
Function GetPropertiesValuesForRef(Ref, PropertiesOfObject, Val ObjectPropertiesString, Val MetadataObjectName) Export
	...
	Except
		MessageString = NStr("en='An error occurred when receiving the reference properties. Query execution failed: %ErrorDescription%';
							 |ru='Ошибка при получении свойств ссылки. Ошибка выполнения запроса: %ErrorDescription%'");
...
Function NodesArrayByPropertiesValues(PropertyValues, Val QueryText, Val ExchangePlanName, Val FlagAttributeName, Val Exporting = False) Export
	...
	Except
		MessageString = NStr("en='An error occurred when receiving a list of target nodes. Failed to execute the query: %ErrorDescription%';
							 |ru='Ошибка при получении списка узлов получателей. Ошибка выполнения запроса: %ErrorDescription%'");
		MessageString = StrReplace(MessageString, "%ErrorDescription%", DetailErrorDescription(ErrorInfo()));
		Raise MessageString;
	EndTry;
...
Procedure ExecuteORRHandlerBeforeProcessing(ORR, Cancel, Object, MetadataObject, Val Exporting)
	...
	If ORR.HasBeforeProcessHandler Then
		...
		|Exchange plan: [ExchangePlanName];
		|Metadata object: [MetadataObjectName]
		|Error description: [Description]';ru='Ошибка при выполнении обработчика: ""[HandlerName]"";
		|План обмена: [ExchangePlanName];
		|Объект метаданных: [MetadataObjectName]
		|Описание ошибки: [Definition]'");
	...
	If ORR.HasOnProcessHandler Then
		...
		Except
			MessageString = NStr("en='An error occurred while executing handler: ""[HandlerName]""; Exchange plan: [ExchangePlanName]; Metadata object: [MetadataObjectName] Error description: [Description]';
								 |ru='Ошибка при выполнении обработчика: ""[HandlerName]""; План обмена: [ExchangePlanName]; Объект метаданных: [MetadataObjectName] Описание ошибки: [Definition]'");
