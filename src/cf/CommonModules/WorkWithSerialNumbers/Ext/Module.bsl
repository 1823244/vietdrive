		...
		For Each StringInventory In Inventory Do
			...
			If StringInventory.ProductsAndServices.UseSerialNumbers Then
				...
				If FilterSerialNumbers.Count() <> RowInventoryQuantity Then
					...
					MessageText = NStr("ru = 'Число серийных номеров табличной части %TSName% отличается от количества единиц в строке %Number%.'; 
										|en = 'The quantity of serial numbers in tabular section %TSName% differs from the quantity of units in the line %Number%.'");
					MessageText = MessageText + NStr("ru = ' Серийных номеров - %QuantityOfNumbers%, нужно %QuantityInRow%';
													|en = 'Serial numbers - %QuantityOfNumbers%, need %QuantityInRow%'");
...
Procedure ExecuteDuplicatesControl(Cancel, Val Inventory, Val SerialNumbers, Val incForm, FieldNameConnectionKey)
	...
	If NOT QueryResult.IsEmpty() Then
		...
		While QueryResultSelection.Next() Do
			MessageText = NStr("ru = 'Серийный номер ""%SerialNumber%"", указанный в строке %LineNumber%, указан повторно.';
								|en = 'Serial number ""%SerialNumber%"", specified in line %LineNumber%, has already been used.'");
