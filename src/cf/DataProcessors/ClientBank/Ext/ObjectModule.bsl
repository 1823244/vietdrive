...
Procedure SetMarkToDelete(ObjectForMark, Check)
	...
	Try
		...
		MessageText = NStr("en='%NameObjectLeft% %NameObjectMid%: %NameOfAction%.';ru='%NameObjectLeft% %NameObjectMid%: %NameOfAction%.'");
		MessageText = StrReplace(MessageText, "%NameObjectLeft%", Upper(Left(NameObject, 1)));
	...
	Except
		MessageText = NStr("en='%NameObjectLeft% %NameObjectMid%: not %NameOfAction%. Errors occurred while writing.';
							|ru='%NameObjectLeft% %NameObjectMid%: не %NameOfAction%! Произошли ошибки при записи!'");
		MessageText = StrReplace(MessageText, "%NameObjectLeft%", Upper(Left(NameObject, 1)));
		MessageText = StrReplace(MessageText, "%NameObjectMid%", Mid(NameObject, 2));
		MessageText = StrReplace(MessageText, "%NameOfAction%", NameOfAction);
		SmallBusinessServer.ShowMessageAboutError(ThisObject, MessageText);
...
Procedure WriteObject(ObjectToWrite, SectionRow, IsNewDocument)
	...
	If ObjectModified Then
		...
		Except
			//( elmi Lost in translation
			//MessageText = NStr("en='%NameObjectLeft% %NameObjectMid% %Status%. Errors occurred while writing.';ru='%НазваниеОбъекта% %NameObjectMid% %Статус%! Произошли ошибки при записи!'");
			MessageText = NStr("en='%NameObjectLeft% %NameObjectMid% %Status%. Errors occurred while writing.';
								|ru='%NameObjectLeft% %NameObjectMid% %Status%! Произошли ошибки при записи!'");
			//) elmi
			MessageText = StrReplace(MessageText, "%NameObjectLeft%", Upper(Left(NameObject, 1)));
			MessageText = StrReplace(MessageText, "%NameObjectMid%", Mid(NameObject, 2));
	...
	If PostImported AND (ObjectModified OR Not ObjectPosted) Then
		...
		Except
			//( elmi Lost in translation
			//MessageText = NStr("en='%NameObjectLeft% %NameObjectMid% is not posted. Errors occurred while posting.';ru='%NameObjectLeft% %НазваниеОбъектаСред% не проведен! Произошли ошибки при проведении!'");
			MessageText = NStr("en='%NameObjectLeft% %NameObjectMid% is not posted. Errors occurred while posting.';ru='%NameObjectLeft% %NameObjectMid% не проведен! Произошли ошибки при проведении!'");
			//) elmi
			MessageText = StrReplace(MessageText, "%NameObjectLeft%", Upper(Left(NameObject, 1)));
			MessageText = StrReplace(MessageText, "%NameObjectMid%", Mid(NameObject, 2));
			SmallBusinessServer.ShowMessageAboutError(ThisObject, MessageText);
			...
			If UseDataProcessorBoundary Then
				If ValueIsFilled(DataProcessorBoundaryDate) Then
					If BegOfDay(DocumentObjectToDeletion.Date) <= BegOfDay(DataProcessorBoundaryDate) Then
						MessageText = NStr("en='The %DocumentObjectToDelete% payment document is not marked for deletion as its date is equal or less than the data processing range.';
										|ru='Платежный документ ""%DocumentObjectToDelete%"" не помечен на удаление, так как имеет дату равной или меньшей границы обработки!'");
