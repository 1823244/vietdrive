	...
	If Quantity < CountTotal Then
		...
		Text = Text
		        ...
		        + NStr("en='Selection mark is set for %QuantitySelected% strings from %QuantityTotal%.';
						|ru='Отметка выбора установлена для %QuantitySelected% строк из %QuantityTotal%.'");
		...
		Text = Text
		        ...
		        + NStr("en='Check: either the quantity is not filled in or the price tag (label) template is not specified. In this case, selection mark for lines cannot be specified.';
						|ru='Проверьте: не заполнены либо количество, либо не задан шаблон ценника (этикетки). В этом случае отметка выбора для строк не может быть установлена.'");
...
Procedure SetPriceTagsQuantityEnd(Result, AdditionalParameters) Export
    ...
    If (Result <> Undefined) Then
        ...
        Text = NStr("en='%Quantity% price tags are set.';ru='Установлено количество ценников %Quantity%.'");
...
Procedure SetLabelsQuantityEnd(Result, AdditionalParameters) Export
    ...
    If (Result <> Undefined) Then
        ...
        Text = NStr("en='%Quantity% labels are set.';ru='Установлено количество этикеток %Quantity%.'");
        Text = StrReplace(Text, "%Quantity%", ValueSelected);
...
Procedure SetPriceTagsTemplateEnd(Result, AdditionalParameters) Export
    ...
    If (Result <> Undefined) Then
        ...
        Text = NStr("en='The %Template% template set.';ru='Установлен шаблон ""%Template%"".'");
...
Procedure SetModeTagsPrintingAtServer()
	...
	If PrintFromDocument Then
		Title = Nstr("en = 'Price tags printing from document'; ru = 'Печать ценников из документа'");
	Else
		Title = Nstr("en = 'Tags printing'; ru = 'Печать ценников'");
	EndIf;
...
Procedure SetModeTagsPrintingAndLabelsAtServer()
	...
	If PrintFromDocument Then
		Title = Nstr("en = 'Printing labels and price tags from document'; ru = 'Печать этикеток и ценников из документа'");
	Else
		Title = Nstr("en = 'Print labels and tags'; ru = 'Печать этикеток и ценников'");
	EndIf;
...
Procedure SetModeLabelsPrintingAtServer()
	...
	If PrintFromDocument Then
		Title = Nstr("en = 'Labels printing from document'; ru = 'Печать этикеток из документа'");
	Else
		Title = Nstr("en = 'Labels printing'; ru = 'Печать этикеток'");
	EndIf;
