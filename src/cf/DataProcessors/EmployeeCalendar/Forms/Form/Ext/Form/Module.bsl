...
Procedure RepresentMyAgenda()
	...
	|		PRESENTATION(WorkOrderWorks.ProductsAndServices),
	...
	|		CASE
	|			WHEN WorkOrderWorks.Ref.State = VALUE(Catalog.EventStates.Completed)
	|				THEN TRUE
	|			ELSE FALSE
	|		END
	|	FROM
...
Procedure RepresentScheduleMonth()
	...
	|			END,
	|			CASE
	|				WHEN NOT WorkOrderWorks.Ref.State = VALUE(Catalog.EventStates.Completed)
