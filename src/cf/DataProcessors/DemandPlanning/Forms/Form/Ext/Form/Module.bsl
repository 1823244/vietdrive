...
Procedure UpdateChoiceListReplenishmentMethod()
	...
	If PurchasesAvailable Then
		If GetFunctionalOption("TransferRawMaterialsForProcessing") AND IsInRole("AddChangeProcessingSubsystem") Then
			Items.FilterReplenishmentMethod.ChoiceList.Add("Purchase and processing", NStr("en = 'Purchase and processing'; ru = 'Закупка и переработка'"));
		Else
			Items.FilterReplenishmentMethod.ChoiceList.Add("Purchase", NStr("en = 'Purchase'; ru = 'Закупка'"));
		EndIf;
	EndIf;
	
	If AvailableProduction Then
		Items.FilterReplenishmentMethod.ChoiceList.Add("Production", NStr("en = 'Production'; ru = 'Производство'"));
		If PurchasesAvailable Then
			Items.FilterReplenishmentMethod.ChoiceList.Add("All", NStr("en = 'All'; ru = 'Все'"));
		EndIf;
	EndIf;
