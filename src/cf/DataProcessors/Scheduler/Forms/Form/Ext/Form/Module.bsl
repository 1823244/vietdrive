...
Function GetScheduleChart()
	...
	|				THEN CASE
	|						WHEN DocumentJournalResourcesPlanningDocuments.OperationKind = VALUE(Enum.OperationKindsCustomerOrder.WorkOrder)
	|							THEN TRUE
	|						ELSE FALSE
	|					END
	|			ELSE TRUE
...
Function GetResourcesWorkImportSchedule(ResourcesList, DaysArray)
	...
	|		WHERE
	|			CustomerOrderEnterpriseResources.Ref.OperationKind = VALUE(Enum.OperationKindsCustomerOrder.WorkOrder)
...
Procedure CreateNewOrderDetailProcessing(Details)
	...
	If WorksScheduleRadioButton = "Day" OR WorksScheduleRadioButton = "4days" Then
		...
		If ShowProductionOrders AND ShowWorkOrders Then
			...
			OrdersList.Add("WorkOrder", "Work-order");
...
Procedure PaintList()
	...
	PaintByStateWorkOrders = Constants.UseCustomerOrderStates.Get();
	...
	SelectionWorkOrderStates = Catalogs.CustomerOrderStates.Select();
	While SelectionWorkOrderStates.Next() Do
		...
		ConditionalAppearanceItem.Presentation = "By work order state " + SelectionWorkOrderStates.Description;
...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	OperationKindWorkOrder = Enums.OperationKindsCustomerOrder.WorkOrder;
...
Procedure AddWorkOrder(Command)
	...
	If Items.Body.CurrentPage = Items.PanelSchedule Then
		...
		If KindChartPlanRadioButton = "Chart" Then
			...
			FillValue.Insert("OperationKind", OperationKindWorkOrder);
