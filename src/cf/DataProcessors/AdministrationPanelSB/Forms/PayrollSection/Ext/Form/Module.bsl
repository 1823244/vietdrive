...
Function CancellationUncheckFunctionalOptionUsePayrollSubsystem()
	...
	If Not ResultsArray[2].IsEmpty() Then
		...
		ErrorText = ErrorText + ?(IsBlankString(ErrorText), "", Chars.LF) + NStr("en='There are documents of the ""Work order"" kind in the infobase that are used to calculate employees'' salary. You cannot clear the ""Salary"" check box.';ru='В информационной базе присутствуют документы ""Заказ - наряд"", которые начисляют зарплату сотрудникам! Снятие флага ""Зарплата"" запрещено!'");
