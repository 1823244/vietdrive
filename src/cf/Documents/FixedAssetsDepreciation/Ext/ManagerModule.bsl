...
Procedure GenerateTableMonthEndErrors(DocumentRefFixedAssetsDepreciation, StructureAdditionalProperties)
	...
	For Each CurRow IN ResultTable Do
		ErrorText = NStr("en='For the %FixedAssetPresentation% (%Code%) property %MessageAboutDepreciationAccrualError%.';
						 |ru='Для имущества %ВнеоборотныйАктивПредставление% (%Код%) %MessageAboutDepreciationAccrualError%.'");
