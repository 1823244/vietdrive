...
Procedure RunPreliminaryControl(Cancel)
	...
	For Each AccrualDetentionRow IN TableAccrualsDeductions Do
		...
		Else
			...
			MessageText = NStr("en='Invalid link condition in row No.%Number% of the ""Accruals and deductions"" tabular section.';
								|ru='Не верное условие связи в строке №%Number% табл. части ""Начислений и удержаний"".'");
	...
	If Not ResultsArray[2].IsEmpty() Then
		...
		While QueryResultSelection.Next() Do
			MessageText = NStr("en='In row No. %Number% of the ""Employees"" tabular section, the order validity contradicts the ""%PersonnelOrder%"" personnel order.';
								|ru='В строке №%Number% табл. части ""Сотрудники"" период действия приказа противоречит кадровому приказу ""%PersonnelOrder%"".'");
	...
	If Not ResultsArray[3].IsEmpty() Then
		...
		While QueryResultSelection.Next() Do
			MessageText = NStr("en='In row No.%Number% of the ""Accruals and deductions"" tabular section, the order validity contradicts the ""%PersonnelOrder%"" personnel order.';
								|ru='В строке №%Number% табл. части ""Начисления и удержания"" период действия приказа противоречит кадровому приказу ""%PersonnelOrder%"".'");
			MessageText = StrReplace(MessageText, "%Number%", QueryResultSelection.LineNumber); 
			MessageText = StrReplace(MessageText, "%PersonnelOrder%", QueryResultSelection.Recorder);
			SmallBusinessServer.ShowMessageAboutError(
				ThisObject,
	...
	If Not ResultsArray[4].IsEmpty() Then
		...
		While QueryResultSelection.Next() Do
			MessageText = NStr("en='In row No.%Number% of the ""Employees"" tabular section, the employee is specified again.';
								|ru='В строке №%Number% табл. части ""Сотрудники"" сотрудник указывается повторно.'");
			MessageText = StrReplace(MessageText, "%Number%", QueryResultSelection.LineNumber);
			SmallBusinessServer.ShowMessageAboutError(
				ThisObject,
				MessageText,
...
Procedure RunControl(AdditionalProperties, Cancel)
	...
	If Not ResultsArray[0].IsEmpty() Then
		...
		While QueryResultSelection.Next() Do
			MessageText = NStr("en='In row No.%Number% of the ""Employees"" tabular section, the %Employee% employee already works in the %StructuralUnit% department.';
								|ru='В строке №%Number% табл. части ""Сотрудники"" сотрудник %Employee% уже работает в подразделении %StructuralUnit%.'");
			MessageText = StrReplace(MessageText, "%Number%", QueryResultSelection.LineNumber); 
	...
	If Not ResultsArray[1].IsEmpty() Then
		...
		While QueryResultSelection.Next() Do
			MessageText = NStr("en='In row No.%Number% of the ""Employees"" tabular section for the %Individual% individual, the %Employee% employee is already hired to a primary place of employment in the %StructuralUnit% department.';
								|ru='В строке №%Number% табл. части ""Сотрудники"" для физлица %Физлицо% уже принят на основное место работы сотрудник %Employee% в подразделение %StructuralUnit%.'");
			MessageText = StrReplace(MessageText, "%Number%", QueryResultSelection.LineNumber); 
	...
	If Not ResultsArray[2].IsEmpty() Then
		QueryResultSelection = ResultsArray[2].Select();
		While QueryResultSelection.Next() Do
			MessageText = NStr("en='In row No.%NumberDouble% of the ""Employees"" tabular section for the %Individual% individual, the %Employee% employee is rehired to a primary place of employment. Individual is already specified in row No. %Number%.';
								|ru='В строке №%NumberDouble% табл. части ""Сотрудники"" для физлица %Физлицо% повторно принимается на основное место работы сотрудник %Employee%. Физлицо уже указано в строке №%Number%.'");
	...
	If Not ResultsArray[3].IsEmpty() Then
		QueryResultSelection = ResultsArray[3].Select();
		While QueryResultSelection.Next() Do
			MessageText = NStr("en='In row No.%Number% of the ""Employee"" tabular section: the employee %Employee% worked in the company earlier. To hire the employee again, create a new employee.';
								|ru='В строке №%Number% табл. части ""Сотрудники"": сотрудник %Employee% уже работал в компании. Для повторного приема на работу необходимо создать нового сотрудника.'");
			MessageText = StrReplace(MessageText, "%Number%", QueryResultSelection.LineNumber); 
			MessageText = StrReplace(MessageText, "%Employee%", QueryResultSelection.Employee);
			SmallBusinessServer.ShowMessageAboutError(
				ThisObject,
...
Procedure RunControlStaffSchedule(AdditionalProperties, Cancel) 
	...
	While Selection.Next() Do
		If Selection.FontsContradiction Then
			MessageText = NStr("en='Row No.%Number% of the ""Employees"" tabular section: employment rates are not provided in the staff list.';
								|ru='Строка №%Number% табл. части ""Сотрудники"": в штатном расписании не предусмотрены ставки для приема сотрудника!'");
