...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Parameters.Property("OperationKindWorkOrder") Then
		FilterValue = Enums.OperationKindsCustomerOrder.WorkOrder;
		ThisForm.AutoTitle = False;
		ThisForm.Title = "Work orders (for payment)";
	Else
...
Procedure CreatePayment(Command)
	...
	Else
		...
		ListPaymentDocuments.Add("CashReceipt", NStr("en='Petty cash receipt'; ru = 'Поступление в кассу'"));
		ListPaymentDocuments.Add("PaymentReceipt", NStr("en='Payment receipt'; ru = 'Поступление на счет'"));
		...
		ListPaymentDocuments.ShowChooseItem(New NotifyDescription("CreatePaymentEnd", ThisObject, New Structure("BasisParameters", BasisParameters)),
			NStr("en='Select payment method'; ru = 'Выберете способ оплаты'"));
	EndIf;
