...
Procedure InitializeDocumentDataWorkOrder(DocumentRefCustomerOrder, StructureAdditionalProperties) 
	...
	|	WorkOrderWorks.ProductsAndServices.BusinessActivity AS BusinessActivitySales,
	|	WorkOrderWorks.ProductsAndServices.BusinessActivity.GLAccountRevenueFromSales AS AccountStatementSales,
	|	WorkOrderWorks.ProductsAndServices.BusinessActivity.GLAccountCostOfSales AS GLAccountCost,
	...
	|	WorkOrderProducts.ProductsAndServices.BusinessActivity AS BusinessActivitySales,
	|	WorkOrderProducts.ProductsAndServices.BusinessActivity.GLAccountRevenueFromSales AS AccountStatementSales,
	|	WorkOrderProducts.ProductsAndServices.BusinessActivity.GLAccountCostOfSales AS GLAccountCost,
...
Procedure CheckAbilityOfEnteringByCustomerOrder(FillingData, AttributeValues) Export
	...
	If AttributeValues.Property("Posted") Then
		If Not AttributeValues.Posted Then
			ErrorText = NStr("en='Document %Document% is not posted. Cannot use the ""input on basis"" method for an unposted document.';ru='Документ %Document% не проведен. Ввод на основании непроведенного документа запрещен.'");
	...
	If AttributeValues.Property("Closed") Then
		If (AttributeValues.Property("WorkOrderReturn") AND Constants.UseCustomerOrderStates.Get())
			...
			If AttributeValues.Closed Then
				ErrorText = NStr("en='The %Document% document is closed (completed). Cannot enter on the basis of the closed order.';ru='Документ %Document% закрыт (выполнен). Ввод на основании закрытого заказа запрещен.'");
				ErrorText = StrReplace(ErrorText, "%Document%", FillingData);
				Raise ErrorText;
			EndIf;
		EndIf;
	...
	If AttributeValues.Property("OrderState") Then
		If AttributeValues.OrderState.OrderStatus = Enums.OrderStatuses.Open Then
			ErrorText = NStr("en='The %Document% document in the %OrderState% state. Cannot enter on the basis.';ru='Документ %Document% в состоянии %OrderState%. Ввод на основании запрещен.'");
			ErrorText = StrReplace(ErrorText, "%Document%", FillingData);
			...
			If AttributeValues.OperationKind = Enums.OperationKindsCustomerOrder.WorkOrder Then
				If AttributeValues.Property("WorkOrderReturn") Then
					If AttributeValues.OrderState.OrderStatus <> Enums.OrderStatuses.Completed Then
						ErrorText = NStr("en='The %Document% document in the %OrderState% state. Cannot enter the return from the customer on the basis.';ru='Документ %Document% в состоянии %OrderState%. Ввод возврата от покупателя на основании запрещен.'");
						ErrorText = StrReplace(ErrorText, "%Document%", FillingData);
						ErrorText = StrReplace(ErrorText, "%OrderState%", AttributeValues.OrderState);
						Raise ErrorText;
					EndIf;
...
Function PrintWorkOrder(ObjectsArray, PrintObjects, TemplateName)
	...
	For Each CurrentDocument IN ObjectsArray Do
		...
		TemplateArea = Template.GetArea("Title");
		TemplateArea.Parameters.HeaderText = "Work order No "
												+ DocumentNumber
												+ " from "
												+ Format(Header.DocumentDate, "DLF=DD");
