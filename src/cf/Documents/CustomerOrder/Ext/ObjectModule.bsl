...
Procedure Filling(FillingData, StandardProcessing)
	...
	If TypeOf(FillingData) = Type("Structure")
		...
		And FillingData.OperationKind = Enums.OperationKindsCustomerOrder.WorkOrder
		...
		EndIf;
		FillingData.Insert("OperationKind", Enums.OperationKindsCustomerOrder.WorkOrder);
...
Procedure BeforeWrite(Cancel, WriteMode, PostingMode)
	...
	If ShipmentDatePosition = Enums.AttributePositionOnForm.InTabularSection Then
		If OperationKind = Enums.OperationKindsCustomerOrder.WorkOrder Then
			ShipmentDate = Finish;
		Else
...
Procedure FillCheckProcessing(Cancel, CheckedAttributes)
	...
	If Constants.FunctionalOptionInventoryReservation.Get() Then
		...
		If OperationKind = Enums.OperationKindsCustomerOrder.OrderForSale Then
			...
			For Each StringInventory IN Inventory Do
				...
				If StringInventory.Reserve > StringInventory.Quantity Then
					...
					MessageText = NStr("en='The number of reserved positions exceeds the total inventory quantity in row No.%Number% of the ""Goods, services"" tabular section.';
										|ru='В строке №%Number% табл. части ""Товары, услуги"" количество резервируемых позиций превышает общее количество запасов.'");
		...
		ElsIf OperationKind = Enums.OperationKindsCustomerOrder.WorkOrder Then
			...
			For Each StringInventory IN Inventory Do
				...
				If OrderState.OrderStatus = Enums.OrderStatuses.Completed
					...
					MessageText = NStr("en='The number of items written off from reserve exceeds the total inventory quantity in row No.%Number% of the Goods tabular section.';
										|ru='В строке №%Number% табл. части ""Товары"" количество списываемых позиций из резерва превышает общее количество запасов.'");
					MessageText = StrReplace(MessageText, "%Number%", StringInventory.LineNumber);
					SmallBusinessServer.ShowMessageAboutError(
						ThisObject,
						MessageText,
				...
				ElsIf StringInventory.Reserve > StringInventory.Quantity Then	
					...
					MessageText = NStr("en='The number of reserved positions exceeds the total inventory quantity in row No.%Number% of the Goods tabular section.';
										|ru='В строке №%Number% табл. части ""Товары"" количество резервируемых позиций превышает общее количество запасов.'");
					MessageText = StrReplace(MessageText, "%Number%", StringInventory.LineNumber);
					SmallBusinessServer.ShowMessageAboutError(
						ThisObject,
						MessageText,
			...
			For Each StringInventory IN Materials Do
				...
				If OrderState.OrderStatus = Enums.OrderStatuses.Completed
					...
					MessageText = NStr("en='Quantity of items written off from reserve exceeds the total inventory quantity in row No.%Number% of the ""Materials"" tabular section.';
										|ru='В строке №%Number% табл. части ""Материалы"" количество списываемых позиций из резерва превышает общее количество запасов.'");
					MessageText = StrReplace(MessageText, "%Number%", StringInventory.LineNumber);
					SmallBusinessServer.ShowMessageAboutError(
						ThisObject,
						MessageText,
				...
				ElsIf StringInventory.Reserve > StringInventory.Quantity Then
					...
					MessageText = NStr("en='The number of reserved positions exceeds the total inventory quantity in row No.%Number% of the ""Materials"" tabular section.';
										|ru='В строке №%Number% табл. части ""Материалы"" количество резервируемых позиций превышает общее количество запасов.'");
					MessageText = StrReplace(MessageText, "%Number%", StringInventory.LineNumber);
					SmallBusinessServer.ShowMessageAboutError(
						ThisObject,
						MessageText,
	...
	If Not Constants.UseCustomerOrderStates.Get() Then
		...
		If Not ValueIsFilled(OrderState) Then
			MessageText = NStr("en='The ""Order state"" field is not filled. Specify state values in the accounting parameter settings.';
								|ru='Поле ""Состояние заказа"" не заполнено. В настройках параметров учета необходимо установить значения состояний.'");
	...
	If ThereAreManualDiscounts OR ThereAreAutomaticDiscounts Then
		For Each StringInventory IN Inventory Do
			...
			If StringInventory.DiscountMarkupPercent <> 100 AND CurAmountDiscounts < CurAmount
				...
				MessageText = NStr("en='The ""Amount"" column is not filled out in the %Number% row of the ""Goods, works, services"" list.';
									|ru='Не заполнена колонка ""Сумма"" в строке %Number% списка ""Товары, работы, услуги"".'");
				MessageText = StrReplace(MessageText, "%Number%", StringInventory.LineNumber);
				SmallBusinessServer.ShowMessageAboutError(
					ThisObject,
					MessageText,
	...
	If ThereAreManualDiscounts Then
		For Each WorkRow IN Works Do
			...
			If WorkRow.DiscountMarkupPercent <> 100 AND CurAmountDiscounts < CurAmount
				...
				MessageText = NStr("en='The ""Amount"" column is not filled out in the %Number% row of the ""Works"" list.';
									|ru='Не заполнена колонка ""Сумма"" в строке %Number% списка ""Работы"".'");
