...
Procedure FillByInvoiceForPayment(FillingData, LineNumber = Undefined, Amount = Undefined)
	...
	If Amount <> Undefined Then
		...
		|					AND DocumentHeader.BasisDocument.OperationKind <> VALUE(Enum.OperationKindsCustomerOrder.WorkOrder)
		|			THEN TRUE
		|		ELSE FALSE
		|	END AS AdvanceFlag,
		|	CASE
	...
	Else
		...
		|					AND DocumentTable.Ref.BasisDocument.OperationKind <> VALUE(Enum.OperationKindsCustomerOrder.WorkOrder)
		|			THEN TRUE
		|		ELSE FALSE
		|	END AS AdvanceFlag,
		|	CASE
...
Procedure FillByCustomerOrder(FillingData, LineNumber = Undefined, Amount = Undefined)
	...
	If Amount <> Undefined Then
		...
		|	DocumentHeader.Contract AS Contract,
		|	CASE
		|		WHEN DocumentHeader.OperationKind = VALUE(Enum.OperationKindsCustomerOrder.WorkOrder)
	...
	Else
		...
		|	DocumentTable.Ref.Contract AS Contract,
		|	CASE
		|		WHEN DocumentTable.Ref.OperationKind = VALUE(Enum.OperationKindsCustomerOrder.WorkOrder)
...
Procedure FillBySalesInvoice(FillingData)
	...
	If FillingData.OperationKind = Enums.OperationKindsCustomerInvoice.TransferToProcessing Then
		ErrorMessage = NStr("en='Cannot enter the document based on the operation - ""%OperationKind"".';
							|ru='Невозможен ввод документа на основании операции - ""%OperationKind""!'");
...
Procedure FillCheckProcessing(Cancel, CheckedAttributes)
	 ...
	 OR OperationKind = Enums.OperationKindsCashReceipt.FromCustomer Then
			...
			For Each RowPaymentDetails IN PaymentDetails Do
				If Not ValueIsFilled(RowPaymentDetails.Document)
				   ...
				   AND Not RowPaymentDetails.AdvanceFlag)) Then
					...
					Else
						If OperationKind = Enums.OperationKindsCashReceipt.FromCustomer Then
							MessageText = NStr("en='Specify the shipment document or payment flag in the %LineNumber% line of the ""Payment explanation"" list.';
												|ru='Укажите документ отгрузки или признак оплаты в строке %НомерСтроки% списка ""Расшифровка платежа"".'");
						Else
							MessageText = NStr("en='Specify document of settlements in the row %LineNumber% of the list ""Payment details"".';
												|ru='Укажите документ расчетов в строке %LineNumber% списка ""Расшифровка платежа"".'");
						EndIf;
		...
		If PaymentAmount <> DocumentAmount Then
			MessageText = NStr("en='Document amount: %DocumentAmount% %CashCurrency% does not correspond to the posted payments in the tabular section: %PaymentAmount% %CashCurrency%!';
								|ru='Сумма документа: %DocumentAmount% %CashCurrency%, не соответствует сумме разнесенных платежей в табличной части: %PaymentAmount% %CashCurrency%!'");
	...
	ElsIf OperationKind = Enums.OperationKindsCashReceipt.OtherSettlements Then
		...
		PaymentAmount = PaymentDetails.Total("PaymentAmount");
		If PaymentAmount <> DocumentAmount Then
			MessageText = NStr("ru = 'Сумма документа: %DocumentAmount% %CashCurrency%, не соответствует сумме разнесенных платежей в табличной части: %PaymentAmount% %CashCurrency%!';
								|en = 'Document amount: %DocumentAmount% %CashCurrency% does not match with the posted payments in the tabular section:  %PaymentAmount% %CashCurrency%!'");
			MessageText = StrReplace(MessageText, "%DocumentAmount%", String(DocumentAmount));
			MessageText = StrReplace(MessageText, "%PaymentAmount%", String(PaymentAmount));
