				...
				If DeviceIdentifier <> Undefined Then
					...
					If Result Then
						...
						Else
							...
							MessageText = NStr("ru = 'При печати чека произошла ошибка.
												...
												|Дополнительное
												|описание: %AdditionalDetails%'; en = 'When printing a receipt, an error occurred.
					...
					Else
						...
						MessageText = NStr("ru = 'При подключении устройства произошла ошибка.
											|Чек не напечатан на фискальном регистраторе.
											|Дополнительное
											|описание: %AdditionalDetails%'; en = 'An error occurred when connecting the device.
											|Receipt is not printed on the fiscal register.
											|Additional
											|description: %AdditionalDetails%'");
	...
	For Each CurrentSettingCommandButtons IN CWPSetting.LowerBarButtons Do
		...
		Except
			...
			Message.Text = StringFunctionsClientServer.SubstituteParametersInString(
				...
				|ru = 'Произошла ошибка при настройке кнопок и команд меню.'"), ErrorDescription());
			Message.Message();
		EndTry;
	EndDo;
...
Procedure BeforeWrite(Cancel, WriteParameters)
	...
	If UseAutomaticDiscounts Then
		If Not Object.DiscountsAreCalculated AND DiscountsChanged() Then
			...
			Message = New UserMessage;
			Message.Text = NStr("en='Automatic discounts (markups) are calculated!'; ru = 'Рассчитаны автоматические скидки (наценки)!'");
...
Procedure SetPeriodAtServer(PeriodKindCWP, ListName, ParameterStandardPeriod = Undefined)
	...
	If PeriodKindCWP = ForCurrentShift Then
		...
		If ListName = "List" Then
			...
			MagazinePeriodKind = NStr("en = 'For current shift';ru = 'За текущую смену'");
		ElsIf ListName = "ReceiptCRListForReturn" Then
			ReceiptCRPeriodKindForReturn = NStr("en = 'For current shift';ru = 'За текущую смену'");
		ElsIf ListName = "ReceiptCRList" Then
			ReceiptCRPeriodKind = NStr("en = 'For current shift';ru = 'За текущую смену'");
		EndIf;
	...
	ElsIf PeriodKindCWP = ForPreviousShift Then
		...
		If ListName = "List" Then
			Items.Date.Visible = False;
			MagazinePeriodKind = NStr("en = 'For last shift';ru = 'За прошлую смену'");
		ElsIf ListName = "ReceiptCRListForReturn" Then
			ReceiptCRPeriodKindForReturn = NStr("en = 'For last shift';ru = 'За прошлую смену'");
		ElsIf ListName = "ReceiptCRList" Then
			ReceiptCRPeriodKind = NStr("en = 'For last shift';ru = 'За прошлую смену'");
		EndIf;
	...
	ElsIf PeriodKindCWP = ForYesterday Then
		...
		If ListName = "List" Then
			Items.Date.Visible = False;
			MagazinePeriodKind = NStr("en = 'For yesterday';ru = 'За вчерашний день'");
		ElsIf ListName = "ReceiptCRListForReturn" Then
			ReceiptCRPeriodKindForReturn = NStr("en = 'For yesterday';ru = 'За вчерашний день'");
		ElsIf ListName = "ReceiptCRList" Then
			ReceiptCRPeriodKind = NStr("en = 'For yesterday';ru = 'За вчерашний день'");
		EndIf;
	...
	ElsIf PeriodKindCWP = ForEntirePeriod Then
		...
		If ListName = "List" Then
			Items.Date.Visible = True;
			MagazinePeriodKind = NStr("en = 'During all the time';ru = 'За всё время'");
		ElsIf ListName = "ReceiptCRListForReturn" Then
			ReceiptCRPeriodKindForReturn = NStr("en = 'During all the time';ru = 'За всё время'");
		ElsIf ListName = "ReceiptCRList" Then
			ReceiptCRPeriodKind = NStr("en = 'During all the time';ru = 'За всё время'");
		EndIf;
...
Function GetPeriodPresentation(StandardPeriod, Delimiter)
	...
	If Not ValueIsFilled(StartDate) AND Not ValueIsFilled(EndDate) Then
		Return NStr("en = 'During all the time';ru = 'За всё время'");;
