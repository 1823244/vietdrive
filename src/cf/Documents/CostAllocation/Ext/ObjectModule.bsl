...
Procedure Filling(FillingData, StandardProcessing) Export
	...
	ElsIf TypeOf(FillingData) = Type("DocumentRef.CustomerOrder")
		AND FillingData.OperationKind = Enums.OperationKindsCustomerOrder.WorkOrder Then	
		
		Raise NStr("en='Cannot enter Cost allocation based on the work order.';ru='Нельзя ввести Распределение затрат на основании заказ-наряда!'");
	...
	If Constants.FunctionalOptionInventoryReservation.Get() Then
		...
		For Each StringInventory IN Inventory Do
			...
			If StringInventory.Reserve > StringInventory.Quantity Then
				...
				MessageText = NStr("en='In row No. %Number% of the """"Inventory"""" tabular section, the number of items for write-off from reserve exceeds the total inventory quantity.';
									|ru='В строке №%Number% табл. части """"Запасы"""" количество позиций к списанию из резерва превышает общее количество запасов.'");
		...
		For Each StringInventoryDistribution IN InventoryDistribution Do
			...
			If StringInventoryDistribution.Reserve > StringInventoryDistribution.Quantity Then
				...
				MessageText = NStr("en='In row No.%Number% of the ""Inventory allocation"" tabular section, the number of items for write-off from reserve exceeds the total inventory quantity.';
									|ru='В строке №%Number% табл. части ""Распределение запасов"" количество позиций к списанию из резерва превышает общее количество запасов.'");
