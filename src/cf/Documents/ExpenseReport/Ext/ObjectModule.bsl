		...
		If PaymentRow.Counterparty.DoOperationsByDocuments
		   ...
		   AND Not ValueIsFilled(PaymentRow.Document) Then
			MessageText = NStr("en='The ""Settlement document"" column is not populated in the %LineNumber% line of the ""Payments"" list.';
								|ru='Не заполнена колонка ""Документ расчетов"" в строке %LineNumber% списка ""Оплаты"".'");
	...
	For Each RowsExpenses IN Expenses Do
		...
		If GetFunctionalOption("AccountingBySeveralDepartments")
		 ...
		 AND Not ValueIsFilled(RowsExpenses.StructuralUnit) Then
			MessageText = NStr("en='The ""Department"" attribute must be filled in for the %ProductsAndServices%"" products and services in the %LineNumber% line of the ""Expenses"" list.';
								|ru='Для номенклатуры ""%ProductsAndServices%"" указанной в строке %LineNumber% списка ""Расходы"", должен быть заполнен реквизит ""Подразделение"".'");
