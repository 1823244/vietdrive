			...
			For Each RowPaymentDetails IN PaymentDetails Do
				If Not ValueIsFilled(RowPaymentDetails.Document)
				   ...
				   AND Not RowPaymentDetails.AdvanceFlag)) Then
					...
					Else
						If OperationKind = Enums.OperationKindsPaymentReceipt.FromCustomer Then
							MessageText = NStr("en='Specify the shipment document or payment flag in the %LineNumber% line of the ""Payment explanation"" list.';ru='Укажите документ отгрузки или признак оплаты в строке %LineNumber% списка ""Расшифровка платежа"".'");
						Else
							MessageText = NStr("en='Specify document of settlements in the row %LineNumber% of the list ""Payment details"".';ru='Укажите документ расчетов в строке %LineNumber% списка ""Расшифровка платежа"".'");
						EndIf;
