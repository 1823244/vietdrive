	...
	For Each String IN AccountsReceivable Do
		If Not ValueIsFilled(String.Document)
			...
			StringComment = NStr("en='It is generated automatically by the ""Entering the opening balances"" document No.%Number% from %Date%';
								 |ru='Сформирован автоматически документом Ввод начальных остатков № %Number% от %Date%'");
...
Procedure FillCheckProcessing(Cancel, CheckedAttributes)
	...
	For Each TSRow IN OtherSections Do
		...
		AND Not ValueIsFilled(TSRow.Currency) Then
			MessageText = NStr("en='The ""Currency"" column is not filled in for the currency account in row No. %LineNumber% of the ""Other sections"" list.';
								|ru='Не заполнена колонка ""Валюта"" для валютного счета в строке %LineNumber% списка ""Прочие разделы"".'");
		...
		AND Not ValueIsFilled(TSRow.AmountCur) Then
			MessageText = NStr("en='The ""Amount (cur.)"" column is not populated for the currency account in the %LineNumber% line of the ""Other sections"" list.';
								|ru='Не заполнена колонка ""Сумма (вал.)"" для валютного счета в строке %LineNumber% списка ""Прочие разделы"".'");
			MessageText = StrReplace(MessageText, "%LineNumber%", String(TSRow.LineNumber));
			SmallBusinessServer.ShowMessageAboutError(
				ThisObject,
				MessageText,
	...
	For Each TSRow IN AccountsReceivable Do
		...
		AND Not ValueIsFilled(TSRow.Document) Then
			MessageText = NStr("en='The ""Settlement document"" column is not populated in the %LineNumber% line of the ""Settlements with customers"" list.';
								|ru='Не заполнена колонка ""Документ расчетов"" в строке %LineNumber% списка ""Расчеты с покупателями"".'");
			MessageText = StrReplace(MessageText, "%LineNumber%", String(TSRow.LineNumber));
			SmallBusinessServer.ShowMessageAboutError(
				ThisObject,
				MessageText,
