...
Procedure FillByCustomerInvoice(FillingData) Export
	...
	Else
		ErrorMessage = NStr("en='Cannot input the ""Receipt"" operation based on the ""%OperationKind%"" operation.';
							|ru='Невозможен ввод операции ""Поступления"" на основании операции - ""%OperationKind%""!'");
		...
		For Each RowsExpenses IN Expenses Do
			...
			If Constants.FunctionalOptionAccountingByMultipleDepartments.Get()
			 ...
			 AND Not ValueIsFilled(RowsExpenses.StructuralUnit) Then
				MessageText = NStr("en='The ""Department"" attribute must be filled in for the ""%ProductsAndServices%"" products and services specified in the %RowNumber% line of the ""Services"" list.';
									|ru='Для номенклатуры ""%ProductsAndServices%"" указанной в строке %LineNumber% списка ""Услуги"", должен быть заполнен реквизит ""Подразделение"".'");
	 ...
	 OR StructuralUnit.StructuralUnitType = Enums.StructuralUnitsTypes.RetailAccrualAccounting Then
		...
		While SelectionOfQueryResult.Next() Do
			...
			MessageText = NStr("en='For products and services %ProductsAndServicesPresentation% in string %LineNumber% of list ""Inventory"" retail price is not set!';
								|ru='Для номенклатуры %ProductsAndServicesPresentation% в строке %LineNumber% списка ""Запасы"" не установлена розничная цена!'");
