...
Procedure FillByStructure(FillingData) Export
	...
	If FillingData.Property("Basis")
		...
		And CommonUse.ObjectAttributeValue(FillingData.Basis, "OperationKind") = Enums.OperationKindsCustomerOrder.WorkOrder
		Then
		Raise NStr("en='Cannot enter invoice on the basis of work order.';ru='Нельзя ввести Расходную накладную на основании заказ-наряда!'");
	EndIf;
	...
	If Constants.FunctionalOptionInventoryReservation.Get()
		...
		For Each StringInventory IN Inventory Do
			...
			If StringInventory.Reserve > StringInventory.Quantity Then
				...
				MessageText = NStr("en='In row No.%Number% of the ""Inventory and services"" tabular section, the quantity of items shipped from the reserve exceeds the total quantity of inventory.';
									|ru='В строке №%Number% табл. части ""Запасы и услуги"" количество отгружаемых позиций из резерва превышает общее количество запасов.'");
		...
		For Each StringInventory IN Inventory Do
			...
			If StringInventory.DiscountMarkupPercent <> 100 AND CurAmountDiscounts < CurAmount
				...
				MessageText = NStr("en='The ""Amount"" column is not populated in the %Number% line of the ""Inventory"" list.';
									|ru='Не заполнена колонка ""Сумма"" в строке %Number% списка ""Запасы"".'");
				MessageText = StrReplace(MessageText, "%Number%", StringInventory.LineNumber);
				SmallBusinessServer.ShowMessageAboutError(
					ThisObject,
					MessageText,
