...
Procedure CreateAcceptanceCertificate(Command)
	...
	Else
		...
		If DataStructure.GenerateFewOrders Then
			...
			MessageText = NStr("en='The orders have different data (%DataPresentation%) in document headers. Create multiple acceptance certificates?';
							   |ru='Заказы отличаются данными (%DataPresentation%) шапки документов! Сформировать несколько актов выполненных работ?'");
