...
Procedure FillByPurchaseInvoice(BasisDocument)
	...
	|							AND NestedSelect.BasisDocument.OperationKind = VALUE(Enum.OperationKindsCustomerOrder.WorkOrder)
	|						THEN NestedSelect.BasisDocument
					...
					Else
						If OperationKind = Enums.OperationKindsCashPayment.Vendor Then
							MessageText = NStr("en='Specify the shipment document or payment flag in the %LineNumber% line of the ""Payment explanation"" list.';
												|ru='Укажите документ отгрузки или признак оплаты в строке %LineNumber% списка ""Расшифровка платежа"".'");
						Else
