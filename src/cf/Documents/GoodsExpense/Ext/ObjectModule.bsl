...
Procedure CheckInputBasedOnPossibility(FillingData, AttributeValues)
	...
	If AttributeValues.Property("OperationKind") Then
		If ValueIsFilled(AttributeValues.OperationKind)
			AND Not FillingData.OperationKind = Enums.OperationKindsCustomerOrder.WorkOrder Then
				ErrorMessage = NStr("en='You can enter issue slip only based on the work order.';ru='Расходный ордер можно ввести только на основании заказ-наряда!'");
				Raise ErrorMessage;
		EndIf;
	EndIf;
	...
	If Cancel Then
		...
		|Документ ""%DocumentRef"" не имеет ордерного склада!'");
