...
Procedure RunPreliminaryControl(Cancel)
	...
	If Not QueryResult.IsEmpty() Then
		...
		While QueryResultSelection.Next() Do
			MessageText = NStr("en='The ""%FixedAsset%"" property in the %LineNumber% line of the ""Property"" list is specified again.';
								|ru='Имущество ""%FixedAsset%"" указанное в строке %LineNumber% списка ""Имущество"", указано повторно.'");
	...
	If TotalOriginalCost <> Amount Then
		MessageText = NStr("en='Products and services amount: %Amount% does not correspond to the amount of initial property costs: %TotalInitialCost%';
							|ru='Стоимость номенклатуры: %Amount% , не соответствует сумме первоначальных стоимостей имущества: %TotalOriginalCost%'");
	...
	For Each RowOfFixedAssets IN FixedAssets Do
		...
		If ArrayVAStatus.Find(RowOfFixedAssets.FixedAsset) <> Undefined Then
			MessageText = NStr("en='The current state of the ""%FixedAsset%"" property specified in the %LineNumber% line of the ""Property"" list is ""Entered in the books"".';
								|ru='Для имущества ""%FixedAsset%"" указанного в строке %LineNumber% списка ""Имущество"", текущее состояние ""Принят к учету"".'");
	...
	For Each RowOfFixedAssets IN FixedAssets Do
		...
		If RowOfFixedAssets.FixedAsset.DepreciationMethod = Enums.FixedAssetsDepreciationMethods.Linear
		   AND RowOfFixedAssets.UsagePeriodForDepreciationCalculation = 0 Then
			MessageText = NStr("en='For property ""%FixedAsset%"" indicated in string %LineNumber% of list ""Property"" should be filled with ""Usage period for depreciation calculation"".';
								|ru='Для имущества ""%FixedAsset%"" указанного в строке %LineNumber% списка ""Имущество"", должен быть заполнен ""Срок использования для вычисления амортизации"".'");
			MessageText = StrReplace(MessageText, "%FixedAsset%", TrimAll(String(RowOfFixedAssets.FixedAsset))); 
			MessageText = StrReplace(MessageText, "%LineNumber%",String(RowOfFixedAssets.LineNumber));
			SmallBusinessServer.ShowMessageAboutError(
				ThisObject,
		...
		If RowOfFixedAssets.FixedAsset.DepreciationMethod = Enums.FixedAssetsDepreciationMethods.ProportionallyToProductsVolume
		   AND RowOfFixedAssets.AmountOfProductsServicesForDepreciationCalculation = 0 Then
			MessageText = NStr("en='For property ""%FixedAsset%"" indicated in string %LineNumber% of list ""Property"" should be filled with ""Product (work) volume for depreciation calculation in physical terms."".';
								|ru='Для имущества ""%FixedAsset%"" указанного в строке %LineNumber% списка ""Имущество"", должен быть заполнен ""Объем продукции (работ) для исчисления амортизации в натуральных ед."".'");
			MessageText = StrReplace(MessageText, "%FixedAsset%", TrimAll(String(RowOfFixedAssets.FixedAsset))); 
			MessageText = StrReplace(MessageText, "%LineNumber%",String(RowOfFixedAssets.LineNumber));
			SmallBusinessServer.ShowMessageAboutError(
				ThisObject,
