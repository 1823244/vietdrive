	...
	While QuerySelection.Next() Do
		...
		Else
			...
			Else
				...
				FillPropertyValues(NewRow, QuerySelection, "ReceiptDocument, ProductsAndServices, Characteristic, Batch, MeasurementUnit, Quantity, VATRate");
...
Procedure ChoiceProcessing(ValueSelected, ChoiceSource)
	...
	For Each ReceiptDocument IN ArrayOfSelectedDocuments Do
		...
		If ArrayOfFoundDocuments.Count() > 0 Then
			...
			MessageText = NStr("en='The %DocumentPerformance% document is already present in the selected document list.';
							   |ru='Документ %DocumentPerformance% уже присутствует в списке выбранных документов.'");
...
Procedure DocumentsMultiplePick(Command)
	...
	ListOfDocumentTypes.Add("ExpenseReport", NStr("en = 'Expense report'; ru = 'Авансовый отчет'"));
	ListOfDocumentTypes.Add("SupplierInvoice", NStr("en = 'Supplier invoice'; ru = 'Приходная накладная'"));
	
	Notification = New NotifyDescription("MultiplePickupDocumentsCompletion",ThisForm);
	ListOfDocumentTypes.ShowChooseItem(Notification, NStr("en = 'Select the document type for add'; ru = 'Выберите тип документа для добавления'"));
