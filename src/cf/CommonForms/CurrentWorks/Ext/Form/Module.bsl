...
Function GetTextOfQueryToCalculateIndicators(IndicatorsGroup)
	...
	ElsIf IndicatorsGroup = "WorkOrders" Then
		...
		Return GetQueryTextForIndicatorWorkOrders();
...
Function GetQueryTextForTargetCustomerOrders()
	...
	|WHERE
	|	DocCustomerOrder.OperationKind <> VALUE(Enum.OperationKindsCustomerOrder.WorkOrder)
...
//
&AtServer
Function GetQueryTextForIndicatorWorkOrders()
	
	QueryText =
	"SELECT ALLOWED
	|	COUNT(DISTINCT CASE
	|			WHEN DocWorkOrder.OrderState.OrderStatus = VALUE(Enum.OrderStatuses.InProcess)
	|					AND DocWorkOrder.Finish < &CurrentDateTimeSession
	|				THEN DocWorkOrder.Ref
	|		END) AS WorkOrdersExecutionExpired,
	|	COUNT(DISTINCT CASE
	|			WHEN DocWorkOrder.SchedulePayment
	|					AND DocWorkOrder.OrderState.OrderStatus = VALUE(Enum.OrderStatuses.InProcess)
	|					AND NOT PaymentSchedule.InvoiceForPayment IS NULL
	|					AND PaymentSchedule.Period < &StartOfDayIfCurrentDateTimeSession
	|				THEN DocWorkOrder.Ref
	|		END) AS WorkOrdersExpiredPayment,
	|	COUNT(DISTINCT CASE
	|			WHEN DocWorkOrder.OrderState.OrderStatus = VALUE(Enum.OrderStatuses.InProcess)
	|					AND DocWorkOrder.Start <= &EndOfDayIfCurrentDateTimeSession
	|					AND DocWorkOrder.Finish >= &CurrentDateTimeSession
	|				THEN DocWorkOrder.Ref
	|			WHEN DocWorkOrder.SchedulePayment
	|					AND DocWorkOrder.OrderState.OrderStatus = VALUE(Enum.OrderStatuses.InProcess)
	...
	|					AND PaymentSchedule.Period = &StartOfDayIfCurrentDateTimeSession
	|				THEN DocWorkOrder.Ref
	|		END) AS WorkOrdersForToday,
	|	COUNT(DISTINCT CASE
	|			WHEN DocWorkOrder.OrderState.OrderStatus = VALUE(Enum.OrderStatuses.InProcess)
	|				THEN DocWorkOrder.Ref
	|		END) AS WorkOrdersInWork
	|FROM
	|	Document.CustomerOrder AS DocWorkOrder
	|		{LEFT JOIN InformationRegister.OrdersPaymentSchedule AS PaymentSchedule
	|		ON DocWorkOrder.Ref = PaymentSchedule.InvoiceForPayment
	|			AND (PaymentSchedule.Period <= &StartOfDayIfCurrentDateTimeSession)}
	|WHERE
	|	DocWorkOrder.OperationKind = VALUE(Enum.OperationKindsCustomerOrder.WorkOrder)
	|	AND DocWorkOrder.Posted
	|	AND NOT DocWorkOrder.Closed
	|	AND DocWorkOrder.Responsible IN(&EmployeesList)"
	;
...
EndProcedure // CustomerWorkOrdersOverduePaymentClick()
...
EndProcedure // CustomerWorkOrdersInProcessClick()
