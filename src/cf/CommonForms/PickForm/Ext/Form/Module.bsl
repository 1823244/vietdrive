...
Procedure FillGenerateFormOpeningParametersWithValuesByDefault(StructureOfParametersByDefault, Parameters)
	...
	// Button OK
	If Parameters.Property("OperationKindWorkOrder") Then
		OperationKindWorkOrder = True;
		Items.OK.Title= "Create " + """Work-order""";
	Else
...
Procedure SetQueryTextOfCharacteristicsList(SettingsStructure)
	...
	// Query parameters
	SetDynamicListParameter(CharacteristicsList,	"PricesPeriod",		Period);
	...
	SetDynamicListParameter(CharacteristicsList,	"PricesKind", 			?(CounterpartyPricesUsed, CounterpartyPriceKind, PriceKind));
...
Procedure OK(Command)
	...
	Else
		...
		If OperationKindWorkOrder Then
			FillStructure.Insert("OperationKind", PredefinedValue("Enum.OperationKindsCustomerOrder.WorkOrder"));
