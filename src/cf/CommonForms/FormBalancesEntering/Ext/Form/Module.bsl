...
Procedure SetExplanationText()
	...
	If mCurrentPageNumber = 0 Then
		Items.DecorationNextActionExplanation.Title = NStr("en = 'Click Next to fill cash funds balance'; ru = 'Нажмите кнопку ""Далее"" для заполнения остатков денежных средств'");
	ElsIf mCurrentPageNumber = 1 Then
		Items.DecorationNextActionExplanation.Title = NStr("en = 'Click Next to fill balances of products'; ru = 'Нажмите кнопку ""Далее"" для заполнения остатков товаров'");
	ElsIf mCurrentPageNumber = 2 Then
		Items.DecorationNextActionExplanation.Title = NStr("en = 'Click Next to fill balance by acounts payable'; ru = 'Нажмите кнопку ""Далее"" для заполнения остатков по расчетам с поставщиками'");
	ElsIf mCurrentPageNumber = 3 Then
		Items.DecorationNextActionExplanation.Title = NStr("en = 'Click Next to fill balance by acounts receivable'; ru = 'Нажмите кнопку ""Далее"" для заполнения остатков по расчетам с покупателями'");
	ElsIf mCurrentPageNumber = 4 Then
		Items.DecorationNextActionExplanation.Title = NStr("en = 'Press ""Next"" to proceed to the final step'; ru = 'Нажмите кнопку ""Далее"" для перехода к итоговому шагу'");
	ElsIf mCurrentPageNumber = 5 Then
		Items.DecorationNextActionExplanation.Title = NStr("en = 'To complete, it is required to click Finish'; ru = 'Для завершения необходимо нажать кнопку ""Закончить""'");
	EndIf;
...
Procedure SetActivePage()
	...
	ThisForm.Title = StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en = 'Opening balance input wizard (Step %1/%2)'; ru = 'Помощник ввода начальных остатков (Шаг %1/%2)'"),
